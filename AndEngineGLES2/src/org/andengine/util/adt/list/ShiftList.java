 ...
 * Its {@link java.util.Queue} like behavior performs better than a plain
 * {@link java.util.ArrayList}, since it automatically shift the contents of its
 * internal Array only when really necessary. Besides sparse allocations to
 * increase the size of the internal Array, {@link ShiftList} is allocation free
 * (unlike the {@link java.util.LinkedList} family).
 * 
...
public class ShiftList<T> implements IQueue<T> {
	...
	private void enterShiftingRight(final int pIndex, final T pItem, final int size) {
		...
		if (shiftAmount == 0) {
			...
			/*
			 * Shift all items to the right of pIndex one to the right, so there
			 * is a free spot at pIndex.
			 */
	...
	private void enterShiftingLeft(final int pIndex, final T pItem) {
		...
		if (pIndex == 0) {
			...
			/*
			 * Shift all items to the left if pIndex one to the left, so there
			 * is a free spot at pIndex.
			 */
	...
	public void shift() {
		...
		if (size == 0) {
			...
			/*
			 * Null out old item references, ensuring not to overwrite just
			 * copied ones.
			 */
